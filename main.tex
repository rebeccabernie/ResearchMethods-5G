...
Our current stations operate on the method of sending signals in multiple directions, rather than on a direct path to the target destination. The incorporation of beamforming and/or spatial multiplexing would not only reduce the likelihood of interference, but would also be much more efficient than current omnidirectional signals.

Spatial multiplexing involves splitting outgoing data into separate packets to be transferred simultaneously, over the same frequency, with the target antenna reassembling the packets upon receipt \cite{beam_sm}. Spatial multiplexing also allows for these packets of data to use walls and other obstacles almost like reflectors to bounce off. This technique would lead to a more efficient data transfer system, one better capable of handling increasing amounts of traffic on the network.

To further improve data transfer methods, the implementation of beamforming would allow antennae to focus data in a directed way, almost like a beam. As discussed in section \textit{\ref{subsec:smallcells}}, researchers at NYU used beamforming to improve tests on indoor use of millimetre waves and small cells. Directional path loss exponents, initially reading 11.6 and 15.6 decibels for 28 and 73GHz bands respectively, were with beamforming reduced to 10.8 and 11.8 decibels, respectively, showing great improvement particularly in 73GHz bands.

As Sun \textit{et al} outlines in \cite{beam_sm}, beamforming coupled with spatial multiplexing is highly appealing considering antennae could be steered in any direction and could bounce data off buildings, leading to very little loss of signal and reduced interference.
\bigskip
...
Complications and questions arise with each major development in any area of technology. Issues at the forefront of disruptions to 5G development include investment, health concerns, cost, and actually building the required infrastructure.

\subsubsection{Investment}It is human nature to want immediate reward for investing in anything, be it stocks or new technologies. 5G development and deployment is an ongoing process, with networks not expected to be implemented on a large scale until after 2020. A recent Gartner Research Circle survey found that three-quarters of participants would be willing to pay more for a 5G service than for 4G \cite{gart5g}, however one can imagine the results would have been significantly less enthusiastic had the question of willingness to invest been proposed.

\subsubsection{Health}As any technology advances and incorporates new ideas, concerns regarding various health risks also develop. Micro-waves, broadcast in the same spectrum range as millimetre waves, have long been the subject of public suspicions. The World Health Organisation concluded in \cite{health} that, on the basis of scientific research, the only affect radio waves (0-6GHz) have on the human body is a minor increase in body temperature from exposure at very high intensities. While existing research suggests millimetre waves will not affect the body significantly more, the WHO's \textit{International EMF Project} aims to continuously evaluate any risks related to waves broadcast frequencies up to 300GHz.
...
There is also a trade off to be acknowledged in the case of rural areas - (will user numbers be worth implementing all the technology?)
